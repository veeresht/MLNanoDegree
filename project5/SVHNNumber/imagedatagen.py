...
################################################################################
""" The following snippet of code is used from
...
class SVHNImageDataGenerator(object):
...
#             save_to_dir=save_to_dir, save_prefix=save_prefix, save_format=save_format)
    ...
    def flow_from_directory(self, directory, metadata_file,
                            target_size=(256, 256), color_mode='rgb',
                            batch_size=32, shuffle=True, seed=131):
        return SVHNDirectoryIterator(
                            directory, metadata_file, self,
...
class SVHNDirectoryIterator(Iterator):

    def __init__(self, data_dir, metadata_file, image_data_generator,
                 target_size=(256, 256), color_mode='rgb',
                 dim_ordering=K.image_dim_ordering(),
        ...
        for fname in sorted(os.listdir(self.directory)):
            ...
            for extension in white_list_formats:
                if fname.lower().endswith('.' + extension):
                    ...
                    break
            if is_valid:
                #self.filenames.append(os.path.join(self.directory, fname))
                self.filenames.append(fname)
                i += 1

        # This hack is needed because the SVHN dataset digitStruct.mat file for
        ...
        for x in data_list:
            if len(x['labels']) == 1:
                self.labels.append([int(x['labels'][0])])
            else:
                self.labels.append(x['labels'])

        super(SVHNDirectoryIterator, self).__init__(self.nb_sample, batch_size, shuffle, seed)
    ...
    def next(self):
        ...
        batch_y = np.zeros([5, batch_x.shape[0], 10], int)
        batch_len = np.zeros([batch_x.shape[0], 7], int)
        for i, j in enumerate(index_array):
            fname = self.filenames[j]
            flabels = self.labels[j]
            flen = len(flabels)
            # for existing digits
            for k in xrange(flen):
                if k < 5:
                    if flabels[k] == 10:
                        ...
                        batch_y[k, i, flabels[k]] = 1.
            # no digit present (class 10)
            # for k in xrange(flen, 5):
            #     batch_y[k, i, 10] = 1.
            if flen <= 5:
                batch_len[i, flen] = 1.
            else:
                batch_len[i, 6] = 1.
        return ({'input_image': batch_x}, {'digit1': batch_y[0],
                                           'digit2': batch_y[1],
                                           'digit3': batch_y[2],
                                           'digit4': batch_y[3],
                                           'digit5': batch_y[4],
