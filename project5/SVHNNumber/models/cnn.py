...
class VGGNetMod_1(object):
    ...
    def define(self, verbose=0):
        ...
        if self.use_batchnorm:
            conv32 = BatchNormalization(mode=0, axis=1)(conv32)
        conv32 = Activation('relu')(conv32)
        conv32 = MaxPooling2D(pool_size=(2, 2), strides=(1, 1),
                              border_mode='same')(conv32)
        ...
        conv41 = Convolution2D(32, 3, 3, border_mode='same',
                               ...
                               init=self.init, subsample=(1, 1))(conv32)
        if self.use_batchnorm:
            conv41 = BatchNormalization(mode=0, axis=1)(conv41)
        conv41 = Activation('relu')(conv41)
        conv41 = MaxPooling2D(pool_size=(2, 2), strides=(2, 2),
                              border_mode='same')(conv41)
        ...
        if self.use_batchnorm:
            conv42 = BatchNormalization(mode=0, axis=1)(conv42)
        conv42 = Activation('relu')(conv42)
        conv42 = MaxPooling2D(pool_size=(2, 2), strides=(1, 1),
                              border_mode='same')(conv42)

        conv_features = Flatten()(conv42)
...
class VGGNetMod_2(object):
    ...
    def define(self, verbose=0):
        ...
        if self.use_batchnorm:
            conv41 = BatchNormalization(mode=0, axis=1)(conv41)
        conv41 = Activation('relu')(conv41)
        conv41 = MaxPooling2D(pool_size=(2, 2), strides=(1, 1),
                              border_mode='same')(conv41)
        ...
        if self.use_batchnorm:
            conv42 = BatchNormalization(mode=0, axis=1)(conv42)
        conv42 = Activation('relu')(conv42)
        conv42 = MaxPooling2D(pool_size=(2, 2), strides=(2, 2),
                              border_mode='same')(conv42)
...
class VGGNetMod_3(object):
    """ Custom Architecture based on VGGNet """

    def __init__(self, model_params, input_dim):

        # Name of the model
        self.name = 'VGGNetMod_3'
        # Weight initialization type
        self.init = model_params['init']
        # Dropout parameters
        self.use_dropout = model_params['use_dropout']
    ...
    def define(self, verbose=0):
        ...
        lc1 = LocallyConnected2D(128, 3, 3, border_mode='valid',
                                 W_regularizer=l2(self.reg_factor),
                                 init=self.init, subsample=(1, 1))(conv42)
        if self.use_batchnorm:
            lc1 = BatchNormalization(mode=0, axis=1)(lc1)
        lc1 = Activation('relu')(lc1)

        conv_features = Flatten()(lc1)
...
class GoogleSVHNNetMod(object):

    def __init__(self, model_params, input_dim):

        # Name of the model
        self.name = 'GoogleSVHNNetMod'
        # Weight initialization type
        self.init = model_params['init']
        # Dropout parameters
        self.use_dropout = model_params['use_dropout']
    ...
    def define(self, verbose=0):
        ...
        # Conv-Relu-MaxPool Layer
        conv11 = Convolution2D(48, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(input_image)
        if self.use_batchnorm:
            conv11 = BatchNormalization(mode=0, axis=1)(conv11)
        ...
        # Conv-Relu-MaxPool Layer
        conv21 = Convolution2D(64, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(conv11)
        if self.use_batchnorm:
            conv21 = BatchNormalization(mode=0, axis=1)(conv21)
        ...
        # Conv-Relu-MaxPool Layer
        conv31 = Convolution2D(128, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(conv21)
        if self.use_batchnorm:
            conv31 = BatchNormalization(mode=0, axis=1)(conv31)
        ...
        # Conv-Relu-MaxPool Layer
        conv32 = Convolution2D(160, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(conv31)
        if self.use_batchnorm:
            conv32 = BatchNormalization(mode=0, axis=1)(conv32)
        conv32 = Activation('relu')(conv32)
        conv32 = MaxPooling2D(pool_size=(2, 2), strides=(2, 2),
                              border_mode='same')(conv32)

        # Conv-Relu-MaxPool Layer
        conv41 = Convolution2D(192, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(conv32)
        if self.use_batchnorm:
            conv41 = BatchNormalization(mode=0, axis=1)(conv41)
        ...
        # Conv-Relu-MaxPool Layer
        conv42 = Convolution2D(192, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(conv41)
        if self.use_batchnorm:
            conv42 = BatchNormalization(mode=0, axis=1)(conv42)
        ...
        # Conv-Relu-MaxPool Layer
        conv43 = Convolution2D(192, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(conv42)
        if self.use_batchnorm:
            conv43 = BatchNormalization(mode=0, axis=1)(conv43)
        conv43 = Activation('relu')(conv43)
        conv43 = MaxPooling2D(pool_size=(2, 2), strides=(1, 1),
                              border_mode='same')(conv43)

        # Conv-Relu-MaxPool Layer
        conv44 = Convolution2D(192, 5, 5, border_mode='same',
                               W_regularizer=l2(self.reg_factor),
                               init=self.init, subsample=(1, 1))(conv43)
        if self.use_batchnorm:
            conv44 = BatchNormalization(mode=0, axis=1)(conv44)
        conv44 = Activation('relu')(conv44)
        conv44 = MaxPooling2D(pool_size=(2, 2), strides=(2, 2),
                              border_mode='same')(conv44)

        # lc1 = LocallyConnected2D(192, 3, 3, border_mode='valid',
        #                          W_regularizer=l2(self.reg_factor),
        #                          init=self.init, subsample=(1, 1))(conv44)
        # if self.use_batchnorm:
        #     lc1 = BatchNormalization(mode=0, axis=1)(lc1)
        # lc1 = Activation('relu')(lc1)

        conv_features = Flatten()(conv44)
        ...
        digit1_pred = Dense(10, activation='softmax', name='digit1')(dense3)
        digit2_pred = Dense(10, activation='softmax', name='digit2')(dense3)
        digit3_pred = Dense(10, activation='softmax', name='digit3')(dense3)
        digit4_pred = Dense(10, activation='softmax', name='digit4')(dense3)
        digit5_pred = Dense(10, activation='softmax', name='digit5')(dense3)
        length_pred = Dense(7, activation='softmax', name='length')(dense3)
